...
# Output: ../data/Agency Insights/declining_agency_watchlist.csv
#
...
def load_from_agency_timeseries(folder: str = "../data/Agency Distribution Over Time") -> pd.DataFrame:
    ...
    files = list(folder_p.glob("*_agency_timeseries*.csv"))
    print(f"[INFO] Looking for inputs in: {folder_p.resolve()}")
    ...
    out = pd.concat(rows, ignore_index=True) if rows else pd.DataFrame(columns=["County","Agency","Time","dist_volume"])
    print(f"[INFO] Loaded {out['County'].nunique()} counties, {out[['County','Agency']].drop_duplicates().shape[0]} county–agency series.")
    return out
# ---------------------------------------------------------------------------
...
def declining_agency_watchlist():
    """
    Writes: ../data/Agency Insights/declining_agency_watchlist.csv
    ...
    out_path = out_dir / "declining_agency_watchlist.csv"

    if df.empty:
        pd.DataFrame(columns=[
            "County","Agency","months_evaluated","recent_window_used",
        ...
        print(f"[OK] Wrote {out_path} (rows=0) — no input data matched.")
        return
    ...
    for (county, agency), g in df.groupby(["County", "Agency"]):
    ...
    watchlist = pd.DataFrame(results).sort_values(
        ["County", "Agency", "flags", "slope_normalized", "median_change_pct"]
    )

    watchlist.to_csv(out_path, index=False)
    print(f"[OK] Evaluated {evaluated} series; flagged {flagged}.")
    print(f"[OK] Wrote {out_path} (rows={len(watchlist)})")
