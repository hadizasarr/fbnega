...
def plot_all_counties():
    ...
    output_dir = "../data/Monthly Plots Top Partner Agencies"
    ...
    for county in unique_counties:
        ...
        unique_agencies = pd.concat([county_df["TopAgency1Name"], county_df["TopAgency2Name"]]).dropna().unique()
        ...
        colors_dict = {agency: color for agency, color in zip(unique_agencies, colors)}
        ...
        plt.bar(county_df["Month"].astype(str), county_df["TopAgency1Weight"], width=0.7, label="Weight", color=county_df["TopAgency1Name"].map(colors_dict))
        
        plt.bar(county_df["Month"].astype(str), county_df["TopAgency2Weight"], width=0.7, 
                bottom=county_df["TopAgency1Weight"], label="Weight", color=county_df["TopAgency2Name"].map(colors_dict))
        
        remainder = county_df["Weight"] - county_df["TopAgency1Weight"] - county_df["TopAgency2Weight"]
        
        plt.bar(county_df["Month"].astype(str), remainder, width=0.7, 
                bottom=county_df["TopAgency1Weight"] + county_df["TopAgency2Weight"], label="Weight", color='lightgray')
        ...
        plt.title(f"{county.title().upper()} - Monthly Weight Distribution with Top 2 Partner Agencies every month")
        plt.xlabel("Month")
        ...
        plt.tight_layout()
        plt.legend(handles=legend_handles, title="Agencies", fontsize="small")
